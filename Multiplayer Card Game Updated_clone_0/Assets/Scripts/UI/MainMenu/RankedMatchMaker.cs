...
public class RankedMatchMaker : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        playerRank = playerInfoManager.playerRank;
    }
    ...
    private async Task<Lobby> JoinLobby()
    {
        try
        {
            Lobby lobby = await Lobbies.Instance.JoinLobbyByIdAsync(playerRank.ToString());
    ...
    private async Task<Lobby> CreateLobby()
    {
        try
        {
            ...
            Lobby lobby = await Lobbies.Instance.CreateLobbyAsync(playerRank.ToString(), 2, options);
